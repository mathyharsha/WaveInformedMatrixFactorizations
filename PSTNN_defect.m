...
% Damage
 ...
 % Undamaged
    ...
    maxP = max(abs(wave_data_undam(:)));
    
    rho = 1;% tune this parameter to control the estimated rank N
    [rankN,~] = prox_rankN(wave_data_undam,rho);%n*ones(1,n3);%
    [Xhat3,~,~,~] =  LRTC_PSTNN(wave_data_,omega,opts,rankN);%,Xhat2);%
    
    Xhat3 = max(Xhat3,0);
    Xhat3 = min(Xhat3,maxP);

    reconstructedData_undam = Xhat3;
