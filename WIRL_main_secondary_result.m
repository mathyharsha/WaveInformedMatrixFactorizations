...
% Uncomment the below to use individual representations from D.
%
...
% 
%     wave_power = sum(power_D(a1:a4,b1:b2,8:12).^2,3);
%     mx = max(wave_power(:));
...
% l1 = min(size(Dc_dam,2),size(x_dam,2));
% l2 = min(size(Dc_undam,2),size(x_undam,2));
% reconstructedData_dam = inv_spectral_wave_transform(Dc_dam(:,1:l1)*x_dam(1:l1),V_x,V_y,V_t);
% reconstructedData_undam = inv_spectral_wave_transform(Dc_undam(:,1:l2)*x_undam(1:l2),V_x,V_y,V_t);
