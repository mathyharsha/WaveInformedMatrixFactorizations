...
n = 200;                  % n should be smaller than the size of WF_original in dim3
...
h=pcolor(flipud(WF_original(:,:,601))); colormap('jet'); title('Original');
...
h=pcolor(flipud(Xt(:,:,601))); colormap('jet'); title('Reconstructed');
h.EdgeColor = 'none'; shading interp;
set(gca, 'XTick', [],'YTick', []);
axis([1 dim2 1 8 min(WF_original(:)) max(WF_original(:)) -max1 max1]);
